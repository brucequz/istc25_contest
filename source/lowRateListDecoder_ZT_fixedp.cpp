...
std::vector<std::vector<LowRateListDecoder::fixedp_cell>> LowRateListDecoder::constructLowRateTrellis_ZT_fixedp(llrvec receivedMessage){
	...
	trellisInfo[0][0].pathMetric = fixedp_type{0};
		...
		for(int currentState = 0; currentState < lowrate_numStates; currentState++){
			...
			for(int forwardPathIndex = 0; forwardPathIndex < numForwardPaths; forwardPathIndex++){
				...
				for(int i = 0; i < lowrate_symbolLength; i++){
					branchMetric += -1 * (receivedMessage[lowrate_symbolLength * stage + i] * fixedp_type(output_point[i]));
				}
...
Fixed_p_MessageInformation LowRateListDecoder::lowRateDecoding_MaxAngle_ProductMetric_ZT_fixedp(llrvec receivedMessage) {
  std::vector<std::vector<fixedp_cell>> trellisInfo;
	trellisInfo = constructLowRateTrellis_ZT_fixedp(receivedMessage);
	...
	// create nodes only for zero state
  ...
  detourTree.insert(detour);
	...
	while(currentAngleExplored < fixedp_type{MAX_ANGLE}){
		...
		if(path[0] == 0 && path[0] == path[lowrate_pathLength - 1] && crc::crc_check(message, crcDegree, crc) && currentAngleExplored <= fixedp_type{MAX_ANGLE}){
